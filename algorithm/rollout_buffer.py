...
class RolloutBuffer:
    def __init__(self, num_steps, num_envs, state_shape, action_shape, device):
        ...
        self.obs = torch.zeros((self.num_steps, self.num_envs, *self.state_shape), dtype=torch.float32, device=device)
        self.actions = torch.zeros((self.num_steps, self.num_envs), dtype=torch.int64, device=device)
        self.logprobs = torch.zeros((self.num_steps, self.num_envs), dtype=torch.float32, device=device)
